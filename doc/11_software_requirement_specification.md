...
- FR-011: 擬似コード編集（セル内ダブルクリックで編集ダイアログ起動、シンタックスハイライト、補完）。
...
## 13. 非機能要件
- 性能: 1 万セル規模の表で操作遅延 100ms 以内（ターゲットマシン: 4C/8G クラス）。ズーム・検索・折畳操作も同水準を維持。
- 起動時間: 3 秒以内（キャッシュビルド後）。
- 信頼性: Undo/Redo 100 ステップ保持。自動保存間隔 30 秒（設定可）。
- 可用性: クラッシュ時に自動復旧可能なスナップショットを保持。
- ユーザビリティ: 主要操作にショートカット提示、ツールチップ提供。キーボード操作のみで表と図の基本編集が完結すること。
